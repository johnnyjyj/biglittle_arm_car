...
void Thumb_up_action(void)
{
   ...
   else if (action_count>=90) Thumb_up_init_value=Thumb_up_init_value+joint_step; //执行的第3段动作
      //输入当前的ros时间
      arm_joint_msg.header.stamp=pub_time;
      //输入机械臂臂身的目标关节角度（单位：弧度）
      arm_joint_msg.position.push_back(0);
...
{ 
    ...
    joints_state_publisher=n.advertise<sensor_msgs::JointState>("/gesture_arm",10);//往控制机械臂运动的话题发布信息
