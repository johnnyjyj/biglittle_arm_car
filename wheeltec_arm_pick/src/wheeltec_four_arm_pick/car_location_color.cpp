...
void car_init::color_location_callback(const wheeltec_arm_pick::four_arm_position &msg)
{
  ...
  if((abs_float(target_liner_x)<0.05)&&(abs_float(target_liner_y)<0.05)&&car_state==0) count++;
  else count=0;
  ...
  if((abs_float(target_liner_x)<0.05)&&(abs_float(target_angular_z)<0.05)&&car_state==0) count++;
  else count=0;
  if(count>=100)  location_flag=1,count=0;//色块定位完成
